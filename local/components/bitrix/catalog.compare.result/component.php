...
if ($arParams["ROOT_SECTION_NAME"] == '')
	$arParams["ROOT_SECTION_NAME"] = GetMessage('ITS_DEFAULT_ROOT_SECTION_NAME');

$arParams['SECTION_ID'] = intval($arParams['SECTION_ID']);
if(isset($_REQUEST['RESET_SECTION']) && $_REQUEST['RESET_SECTION'] == 'Y') {
...
if (!empty($arCompare) && is_array($arCompare))
{
	...
	//EXECUTE
    $arResult['SECTIONS'][0] = ['NAME' => $arParams["ROOT_SECTION_NAME"], 'SELECTED' => true, 'ID' => null];
    $sectionRoots = [];
    $sectionSelected = 0;
	...
	while($obElement = $rsElements->GetNextElement())
	{
        ...
        if( !in_array($arItem['IBLOCK_SECTION_ID'], array_keys($sectionRoots)) ) {
            $sectionRoots[$arItem['IBLOCK_SECTION_ID']] =
                CIBlockSection::GetNavChain($arParams['IBLOCK_ID'], $arItem['IBLOCK_SECTION_ID'], ['NAME', 'ID'])->Fetch();
            $sectionRoots[$arItem['IBLOCK_SECTION_ID']]['SELECTED'] = $sectionRoots[$arItem['IBLOCK_SECTION_ID']]['ID'] == $arParams['SECTION_ID'];
        }
        $section = $sectionRoots[$arItem['IBLOCK_SECTION_ID']];
        if( !isset($arResult['SECTIONS'][$section['ID']]) ) $arResult['SECTIONS'][$section['ID']] = $section;
        if($section['SELECTED']) {
            $sectionSelected = $section['ID'];
        }

        if(!$section['SELECTED'] && $arParams['SECTION_ID'] > 0) continue;
		...
		if (count($arResult["ITEMS"]) >= $arParams['PAGE_ELEMENT_COUNT']) break;
	}
	$arResult['SELECTED_SECTION'] = $sectionSelected;
    if($sectionSelected > 0) $arResult['SECTIONS'][0]['SELECTED'] = false;
	if(!isset($arResult['SECTIONS'][$arParams['SECTION_ID']])) {
	    if(isset($_REQUEST['ajax_action']) && $_REQUEST['ajax_action'] == 'Y') {
            $arResult['RESET_SECTION'] = 'Y';
        } else {
            LocalRedirect('?RESET_SECTION=Y');
        }
//        array_unshift($arResult['SECTIONS'], ['NAME' => GetMessage('ITS_DEFAULT_BAD_SECTION_MSG'), 'SELECTED' => true]);
//        $arResult['SECTIONS'][$sectionSelected]['SELECTED'] = false;
    }
