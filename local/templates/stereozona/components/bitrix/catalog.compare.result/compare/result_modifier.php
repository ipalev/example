...
if ($existShow || $existDelete)
{
    ...
    Collection::sortByColumn($arResult['ALL_OFFER_PROPERTIESALL_OFFER_PROPERTIES'], array('SORT' => SORT_ASC, 'ID' => SORT_ASC));
}

$displayProperties = [];
foreach ($arResult["ALL_PROPERTIES"] as $key => $arProperty) {
    if ($arResult['DIFFERENT']) {
        $arCompare = [];
        foreach($arResult["ITEMS"] as $arElement)
        {
            $arPropertyValue = $arElement["DISPLAY_PROPERTIES"][$key]["VALUE"];
        ...
        if (count(array_unique($arCompare)) == 1) continue;
    }
    if (in_array($arProperty['PROPERTY_TYPE'], ['L', 'N', 'S', 'E'])) {
...
$arResult["ALL_PROPERTIES"] = $displayProperties;

$brands = [];
foreach ($arResult['ITEMS'] as &$arItem) {
    $brands[$arItem['PROPERTIES']['BREND']['VALUE']] = $arItem['PROPERTIES']['BREND']['VALUE'];
}
$arBrands = \Bitrix\Iblock\ElementTable::query()
    ->setSelect(['ID', 'NAME'])
    ->whereIn('ID', $brands)
    ->fetchAll();
foreach ($arBrands as $arBrand) {
    $brands[$arBrand['ID']] = $arBrand['NAME'];
}

foreach ($arResult['ITEMS'] as &$arItem) {
    $arItem['PRICE'] = number_format(current(current($arItem['PRICE_MATRIX']['MATRIX']))['PRICE'], 0, '.', ' ') . '₽';
    $arItem['PICTURE_SRC'] = CFile::GetPath(current($arItem['PROPERTIES']['GALLERY']['VALUE']));
    $arItem['PROPERTIES']['BREND']['VALUE'] = $brands[$arItem['PROPERTIES']['BREND']['VALUE']];
    $arItem['DISPLAY_PROPERTIES'] = [];
    foreach ($displayProperties as $key => $arProperty) {
        $arProperty['VALUE'] = $arItem['PROPERTIES'][$key]['VALUE'];
        if (is_array($arProperty['VALUE'])) {
            $arProperty['VALUE'] = implode(', ', $arProperty['VALUE']);
        }
        $arItem['DISPLAY_PROPERTIES'][] = $arProperty;
    }
}