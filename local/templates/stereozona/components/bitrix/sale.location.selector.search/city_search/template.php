...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <? if($arResult['MODE_CLASSES'] <> ''): ?> <?= $arResult['MODE_CLASSES'] ?><? endif?>">
		...
		<div class="<?=$dropDownBlock?> bx-ui-sls-input-block modal__city-form">
			...
			<input
				...
				class="dropdown-field modal__city-input"
		...
		<div class="bx-ui-sls-error-message">
			...
			<?endif?>
				...
				<div class="modal__city-geo-wrapper">
					<a class="modal__city-geo" data-no-swup>
							Определить по геолокации
					</a>
					<span class="modal__city-geo-error">К сожалению, определить не получилось</span>
				</div>
        
        <?if(is_array($arResult['DEFAULT_LOCATIONS']) && !empty($arResult['DEFAULT_LOCATIONS'])):?>
            <div class="bx-ui-sls-quick-locations quick-locations modal__city-list">
                <?foreach($arResult['DEFAULT_LOCATIONS'] as $lid => $loc):?>
                    <a
                        href="#"
                        data-id="<?=intval($loc['ID'])?>"
                        data-no-swup
                        class="quick-location-tag"
                    >
                        <?=htmlspecialcharsbx($loc['NAME'])?>
                    </a>
                <?endforeach?>
            </div>
				<?endif?>
