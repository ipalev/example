...
$_SERVER["DOCUMENT_ROOT"] = "/var/www/webadmin/data/www/stereozona.ru";
#file_put_contents($_SERVER["DOCUMENT_ROOT"] ."/local/cron/.sitemap.txt", date("d.m.Y H:i:s"));
...
function seoSitemapGetFilesData($PID, $arSitemap, $arCurrentDir, $sitemapFile)
{
  ...
  if ($arCurrentDir['ACTIVE'] == SitemapRuntimeTable::ACTIVE) {
    $list = \CSeoUtils::getDirStructure(
      $arSitemap['SETTINGS']['logical'] == 'Y', $arSitemap['SITE_ID'], $arCurrentDir['ITEM_PATH']
    );
        ...
        if ($checked == 'Y') {
          if (strncmp($arCurrentDir['ITEM_PATH'], $dirKey, $len) === 0) {
            $fileName = IO\Path::combine(
              SiteTable::getDocumentRoot($arSitemap['SITE_ID']), $dirKey
            );
      ...
      foreach ($arIBlockList as $iblockId => $iblockActive) {
        if ($iblockActive !== 'Y' || !array_key_exists($iblockId, $arIBlocks)) {
          ...
          $fileName = str_replace(
            array('#IBLOCK_ID#', '#IBLOCK_CODE#', '#IBLOCK_XML_ID#'), array($iblockId, $arIBlocks[$iblockId]['CODE'], $arIBlocks[$iblockId]['XML_ID']), $arSitemap['SETTINGS']['FILENAME_IBLOCK']
          );
    ...
    if (!$currentIblock) {
      ...
      if ($currentIblock) {
        ...
        if (!$arCurrentIBlock) {
          ...
          $fileName = str_replace(
            array('#IBLOCK_ID#', '#IBLOCK_CODE#', '#IBLOCK_XML_ID#'), array($iblockId, $arCurrentIBlock['CODE'], $arCurrentIBlock['XML_ID']), $arSitemap['SETTINGS']['FILENAME_IBLOCK']
          );
          $sitemapFile = new    SitemapRuntime($PID, $fileName, $arSitemapSettings);
        }
      }
      ...
      if ($dbIblockResult == null) {
        if (isset($NS['CURRENT_SECTION'])) {
          $dbIblockResult = CIBlockElement::GetList(
            ...
            'SITE_ID' => $arSitemap['SITE_ID'],
          ), false, array('nTopCount' => 1000), array('ID', 'TIMESTAMP_X', 'DETAIL_PAGE_URL')
          );
        } else {
          $NS['LAST_ELEMENT_ID'] = 0;
        ...
        if ($bActive) {
          if ($dbTopicResult == null) {
            $dbTopicResult = CForumTopic::GetList(
              array("LAST_POST_DATE" => "DESC"), array_merge(
              array(
                "FORUM_ID" => $forumId,
                "APPROVED" => "Y"), ($NS['FORUM_CURRENT_TOPIC'] > 0 ? array(
              ">ID" => $NS["FORUM_CURRENT_TOPIC"]
            ) : array()
            )
            ), false, 0, array('nTopCount' => 100)
            );
          }
          if (($arTopic = $dbTopicResult->fetch()) && $arTopic) {
            $NS["FORUM_CURRENT_TOPIC"] = $arTopic["ID"];
            $url = CForumNew::PreparePath2Message(
              $arCurrentForum["PATH2FORUM_MESSAGE"], array(
    ...
    if (count($NS['XML_FILES']) > 0) {
      foreach ($NS['XML_FILES'] as $xmlFile) {
        $arFiles[] = new    IO\File(IO\Path::combine(
          $sitemapFile->getSiteRoot(), $xmlFile
        ), $arSitemap['SITE_ID']);
      }
    }
